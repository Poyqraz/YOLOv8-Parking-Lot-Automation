        ...
        if result.boxes is not None:
            for box in result.boxes:
                ...
                cv2.putText(frame, label, (int(xmin + (xmax - xmin) / 2 - len(label) * 3), int(ymin - 10)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.22, (255, 255, 1), 2)
                ...
                if cls == 1:  # Assuming class 0 is "bos"
                    ...
                    bos_counter += 1
                    bos_name = f"A{bos_counter}"

                    # Write the unique name below the detection box
                    cv2.putText(frame, bos_name, (int(xmin), int(ymax + 20)),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 1), 2)

    # Nesne1 detection message
    if bos_detected:
        cv2.putText(frame, "bos alan tespit edildi", (int(frame.shape[1] / 2 - 100), int(frame.shape[0] - 40)),
